...
function setupEventHandlers() {
  ...
  raceCards.forEach(card => {
    card.addEventListener('click', function() {
      ...
      if (raceName) {
        ...
        alert('Halaman Masih Kosong ');
      }
    });
  });

  // Event handling untuk driver cards
  const driverCards = document.querySelectorAll('.driver-card');
  console.log('Jumlah driver cards ditemukan: ' + driverCards.length);
  
  driverCards.forEach(card => {
    card.addEventListener('click', function() {
      const driverName = this.querySelector('.driver-name');
      const teamName = this.querySelector('.team-name');
      if (driverName && teamName) {
        console.log('Driver: ' + driverName.textContent + ' - Team: ' + teamName.textContent);
        alert('Halaman Masih Kosog');
      }
    });
  });

  // Event handling untuk team cards
  const teamCards = document.querySelectorAll('.team-card');
  console.log('Jumlah team cards ditemukan: ' + teamCards.length);
  
  teamCards.forEach(card => {
    card.addEventListener('click', function() {
      const teamNameEl = this.querySelector('.team-name');
      const driversEl = this.querySelector('.team-drivers');
      if (teamNameEl && driversEl) {
        console.log('Team: ' + teamNameEl.textContent + ' - Drivers: ' + driversEl.textContent);
        alert('Halaman Masih Kosong');
      }
    });
  });
  ...
  if (heroButton) {
    ...
    heroButton.addEventListener('click', function(e) {
      console.log('Button Tentang Kami diklik');
    });
  ...
  // Tambahkan animasi pada semua card
  const allCards = document.querySelectorAll('.race-card-full, .driver-card, .team-card');
  allCards.forEach(card => addCardAnimation(card));
}
